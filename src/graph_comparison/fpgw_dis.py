...
# # def adj_to_geodesic_cost(A, weight_as_similarity=True):
...
# # ):
# #     """
# #     Build q for partial graph so it remains valid even when some partial nodes
# #     do not exist in the complete graph.

# #     Strategy:
# #     - overlapping partial nodes inherit mass from complete graph
# #     - non-overlapping partial nodes get mass from target-side structure
# #     - then combine both and normalize
# #     """
...
# #     q = inherit_weight * inherited + (1.0 - inherit_weight) * target_prior
...
# # ):
# #     """
# #     Estimate how much transport mass should be allowed based on node overlap.
# #     """
...
# # # ─────────────────────────────────────────────
# # # Soft Weighted Coverage via γ
# # # ─────────────────────────────────────────────

# # def soft_weighted_coverage(gamma, p, node_importance):
# #     """
# #     gamma: (n_complete, n_partial)
# #     p:     (n_complete,)
# #     node_importance: importance on complete/reference nodes
# #     """
# #     outgoing = gamma.sum(axis=1)
# #     ratio = outgoing / (p + 1e-12)
# #     ratio = np.minimum(ratio, 1.0)

# #     return float(np.sum(node_importance * ratio))


# # # ─────────────────────────────────────────────
# # # MAIN: compatible FLGW + Weighted Coverage
# # # ─────────────────────────────────────────────

# # def flgw_weighted_coverage(
# #     A_ref,
# #     A_tgt,
# #     complete_node_ids,
# #     partial_node_ids,
...
# #         # ── Cost matrices
# #         C_ref = adj_to_geodesic_cost(A_ref)
# #         C_tgt = adj_to_geodesic_cost(A_tgt)
...
# #         )
# #         gamma = log["T"]
# #         coverage = soft_weighted_coverage(gamma, p, p)
# #         final_score = coverage
# #         return float(final_score), gamma, p , q
...
# # ─────────────────────────────────────────────
# # Affinity-Weighted Soft Coverage via γ
# # ─────────────────────────────────────────────

# def affinity_weighted_coverage(gamma, p, node_importance, M, beta=2.0):
#     """
#     Computes soft coverage by weighting the transported mass by a feature affinity.
    
#     gamma:           (n_complete, n_partial) transport plan
#     p:               (n_complete,) marginal mass of complete graph
#     node_importance: (n_complete,) structural importance for scoring 
#     M:               (n_complete, n_partial) feature cost matrix
#     beta:            decay parameter for the heat kernel affinity
#     """
#     # 1. Convert feature costs into affinities (1.0 for perfect match, approaches 0 for high cost)
#     A_affinity = np.exp(-beta * M)
    
#     # 2. Weight the transported mass by match quality
#     weighted_gamma = gamma * A_affinity
    
#     # 3. Sum the quality-weighted mass mapped to each candidate node
#     outgoing = weighted_gamma.sum(axis=1)
    
#     # 4. Calculate ratio of explained mass
#     ratio = outgoing / (p + 1e-12)
#     ratio = np.minimum(ratio, 1.0) # Cap at 1.0 for numerical stability
    
#     # 5. Aggregate overall soft coverage score
#     return float(np.sum(node_importance * ratio))


# # ─────────────────────────────────────────────
# # MAIN: compatible PFGW + Affinity Coverage
# # ─────────────────────────────────────────────
...
#     partial_node_ids,
#     alpha=0.5,
#     beta=2.0,                  # Added beta parameter for affinity scaling
#     coverage_weight=1.0,
#     inherit_weight=0.7,
#     mismatch_cost=1.0,
#     numItermax=1000,
...
#         # ── Compute Affinity-Weighted Coverage
#         coverage = affinity_weighted_coverage(gamma, p, p, M, beta=beta)
        
#         final_score = coverage * coverage_weight
...
def compute_partial_importance_compatible(
    ...
    if inherited_sum > 1e-12:
        inherited = inherited / inherited_sum
    return inherited
    # FIXED: Restored the blend so novel partial nodes don't get zero mass
    # q = inherit_weight * inherited + (1.0 - inherit_weight) * target_prior
